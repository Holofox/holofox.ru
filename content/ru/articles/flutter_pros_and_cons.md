...
У любого инструмента появляются [слабые места](https://github.com/flutter/flutter/wiki/Popular-issues), с которыми
сталкиваешься только тогда, когда начинаешь подходить к решению
серьёзных задач.
...
он не получит тот самый первоклассный пользовательский опыт от его использования. Физика и анимации лишь пытаются быть
похожими на те, к которым привык пользователь.

Разработчикам всё же удалось бесшовно повторить компоненты и физику в
стиле [Material Design](https://m3.material.io/), за
...
Если говорить о веб-приложениях, то разработка на [Flutter Web](https://flutter.dev/multi-platform/web) будет требовать
особый и строгий подход
...
производительность может значительно уступать другим инструментам, что в итоге негативно сложится на общем впечатлении
...
есть [возможность](https://medium.com/kbtg-life/adding-apple-watch-to-flutter-app-via-flutter-method-channel-f1443532d94e)
встроить собственное нейтив расширение в текущее приложение **Flutter**.
...
использовать [Dart Code Metrics](https://dcm.dev/docs/individuals/getting-started), но в больших проектах будет
...
платформ. Обычно выбирается что-то среднее и чаще всего основой в дизайне
выступает [Human Interface Guidelines](https://developer.apple.com/design/human-interface-guidelines/guidelines/overview/)
. Этим часто злоупотребляют, что приводит к галактическим проблемам.
...
особенности, через некоторое время всё может [измениться](https://github.com/flutter/flutter/wiki/Roadmap) настолько,
что текущие минусы больше не будут иметь значимость,
